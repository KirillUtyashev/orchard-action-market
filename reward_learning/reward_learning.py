...
from helpers.helpers import (
    ...
    step_and_evaluate_reward_prediction_accuracy_decentralized,
)
...
class RewardLearning(Algorithm, ABC):
    ...
    def build_experiment(
        ...
        **kwargs,
        ...
        self.network_for_eval = [agent.reward_network for agent in self._agents_list]
...
class RewardLearningCentralized(RewardLearning):
    ...
    def collect_observation(self, step: int) -> None:
        try:
            for tick in range(self.train_config.num_agents):
                ...
                ].reward_network.add_experience(state, reward_sum)
                if (
                    self.train_config.new_dynamic
                    and np.sum(env_step_result.reward_vector) > 0
                ):
    ...
    def process_accuracy(self, agents_list):
        ...
        for key in self._agents_list[0].prediction_accuracy_by_reward.keys():
            if agents_list[0].total_predictions_by_reward[key] != 0:
                self._agents_list[0].prediction_accuracy_by_reward[key].append(
                    agents_list[0].correct_predictions_by_reward[key]
                    / agents_list[0].total_predictions_by_reward[key]
                )
            else:
    ...
    def _evaluate_final(self) -> Tuple[np.floating, ...]:
        ...
        self.logger.info(
            f"Final accuracy: {self._agents_list[0].prediction_accuracy_history[-1]}"
        )

        # Average accuracy over rewards
        avg_acc_by_reward = {"-1.0": 0, "0.0": 0, "1.0": 0, "other": 0}
        for key in self._agents_list[0].prediction_accuracy_by_reward.keys():
            avg_acc_by_reward[key] = self._agents_list[0].prediction_accuracy_by_reward[
                key
            ][-1]
...
class RewardLearningDecentralized(RewardLearning):
    ...
    @property
    def agents_list(self) -> Sequence[RewardAgent]:
        """
        Overrides the base property to provide a more specific type hint
        for static analysis and autocomplete within this subclass.
        """
            ...
            for tick in range(self.train_config.num_agents):
                ...
                for each_agent in range(len(self.agents_list)):
                    ...
                    if (
                        ...
                        self.same_cell_no_reward.append(
                            self.same_cell_no_reward[-1] + 1
                        )
        ...
        for num, agent in enumerate(agents_list):
            ...
            for key in agent.prediction_accuracy_by_reward.keys():
                if agent.total_predictions_by_reward[key] != 0:
                    self.agents_list[num].prediction_accuracy_by_reward[key].append(
                        agent.correct_predictions_by_reward[key]
                        / agent.total_predictions_by_reward[key]
                    )
                else:
        ...
        with torch.no_grad():
            try:
                results = eval_performance(
                    ...
                    timesteps=10000,
                    epsilon=self.train_config.epsilon,
                    env_step=step_and_evaluate_reward_prediction_accuracy_decentralized,
                )
            except Exception as e:
