...
def parse_args(args):
    ...
    algorithm_choices = [
        "Centralized",
        "Decentralized",
        "DecentralizedPersonal",
        "ActorCritic",
        "ActorCriticRates",
        "ActorCriticNoAdvantage",
        "ActorCriticRatesFixed",
        "ActorCriticRatesAdvantage",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10000,
        help="Number of timesteps for each evaluation run used in inference.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--env_cls",
        type=str,
        default="OrchardBasic",
        help="Environment Class",
        choices=env_cls_choices,
    )
...
def main(args):
    ...
    algo: Optional[Algorithm] = pick_experiment(args.algorithm, exp_config)
    if algo is None:
        exit(1)
...
def pick_experiment(algorithm, exp_config):
    if algorithm == "Centralized":
        ...
        algorithm == "ActorCriticRates"
    ):  # QUESTION: what are these rates, noadvantage, ratesfixed, etc. A: they are not important mostly.
