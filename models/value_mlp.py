...
class ValueMLPCentralized(BaseValueModel):
    def __init__(
        self, height: int, width: int, lr: float, discount: float,
        hidden_dim: int = 512, num_layers: int = 4, replay_buffer_capacity: int = 100000
    ):
    ...
    def raw_state_to_nn_input(self, state: State, **kwargs) -> np.ndarray:
        ...
        return np.concatenate([state.apples.flatten(), state.agents.flatten()]).astype(np.float32)
