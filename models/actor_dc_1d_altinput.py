...
class SimpleConnectedMultiple(nn.Module):
    def __init__(self, oned_size): # we work with 1-d size here.
        ...
        self.layer1 = nn.Linear((oned_size * 2 - 1) * 2, 128)
        self.layer2 = nn.Linear(128, 128)
        self.layer3 = nn.Linear(128, 128)
        self.layer4 = nn.Linear(128, 3) # [0] is move left, [1] is move right, [2] is don't move
    ...
    def forward(self, a, b, pos):
        # create "view" vector:
        # x_test = torch.cat((a.flatten(), b.flatten(), pos.flatten()))
        # print(x_test)
        pos1 = int(pos.cpu().item())
        buffer2 = torch.tensor(np.array([-1] * pos1), device=torch.device('cuda'))
        buffer1 = torch.tensor(np.array([-1] * (self.size - 1 - pos1)), device=torch.device('cuda'))
        x1 = torch.cat((buffer1, a.flatten(), buffer2))
        x2 = torch.cat((buffer1, b.flatten(), buffer2))
        # print(x1, x2)
        # print("===")
        # print(x1)
        # print(x2)
