...
class CNNCoordConv(nn.Module):
    def __init__(self, input_channels, height, width, conv_channels, hidden_dim, num_layers, kernel_size=1):
        super().__init__()
        self.height = height
        self.width = width
    ...
    def forward(self, x):
        ...
        # Explicit expansion
        yy = self.yy.expand(batch_size, 1, self.height, self.width)
        xx = self.xx.expand(batch_size, 1, self.height, self.width)
