...
def _make_pick_cfg(n_agents=2, n_task_types=2, relatedness_width=0, proficiency_width=0) -> EnvConfig:
    return EnvConfig(
...
class TestComputePickRewards:
    def test_default_params_self_reward_only(self):
        """With relatedness_width=0, proficiency_width=0, sigma_a=0, sigma_b=0:
        phi[i, kappa] = 1 iff i == kappa (proficiency_width=0 → S=0 → |i-κ|<=0)
        R[i, j] = 1 iff i == j (relatedness_width=0 → C=0 → |i-j|<=0)
        ...
        """
        set_all_seeds(0)
        cfg = _make_pick_cfg(n_agents=2, n_task_types=2, relatedness_width=0, proficiency_width=0)
    ...
    def test_wrong_type_zero_reward(self):
        """phi[0, 1] = 0 when proficiency_width=0 → all rewards are 0."""
        set_all_seeds(0)
        cfg = _make_pick_cfg(n_agents=2, n_task_types=2, relatedness_width=0, proficiency_width=0)
        env = StochasticEnv(cfg)
    ...
    def test_relatedness_width_spreads_reward_to_groupmates(self):
        """With relatedness_width=1, agents 0 and 1 are teammates (|0-1|<=1).
        So reward spreads to both from a pick by actor=0.
        """
        set_all_seeds(0)
        cfg = _make_pick_cfg(n_agents=2, n_task_types=2, relatedness_width=1, proficiency_width=1)
        env = StochasticEnv(cfg)

        rewards = env._compute_pick_rewards(actor=0, tau=0)
        # With relatedness_width=1: R[0,0]=1, R[0,1]=1
        # With proficiency_width=1: phi[0,0]=1, phi[0,1]=1
