...
OUT_DIR="${OUT_DIR:-${EXP_DIR}/supervised_reward_eval_eps_nearest_rel15}"

REL_VALUES=(5 1)
PROF="${PROF:-5}"
POLICY="${POLICY:-eps_nearest}"

REWARD_LRS=(${REWARD_LRS:-3e-3 1e-2 3e-2})
MODEL_TAGS=(${MODEL_TAGS:-cnn32_mlp32 cnn32_nomlp cnn16_mlp16 cnn16_nomlp})
CONV_SPECS=("[[32,3],[32,3]]" "[[32,3],[32,3]]" "[[16,3],[16,3]]" "[[16,3],[16,3]]")
MLP_DIMS=("[32]" "[]" "[16]" "[]")
...
NUM_LRS=${#REWARD_LRS[@]}
TOTAL_RUNS=$((NUM_RELS * NUM_MODELS * NUM_LRS))
...
echo "Policy: ${POLICY}, behavior LR: ${LR_LABEL}"
...
ensure_cache() {
    local rel="$1"
    local cache="${OUT_DIR}/rel${rel}_prof${PROF}_${POLICY}_rollout.pkl"
    local cache_args=()
...
if [ "${DEVICE}" = "cuda" ]; then
    ...
    export CUDA_MPS_PIPE_DIRECTORY="${TMPDIR:-/tmp}/mps_reward_eps_${SLURM_JOB_ID}_${TASK_ID}"
    export CUDA_MPS_LOG_DIRECTORY="${TMPDIR:-/tmp}/mps_reward_eps_${SLURM_JOB_ID}_${TASK_ID}"
...
for ((IDX=START_INDEX; IDX<END_INDEX; IDX++)); do
    REL_INDEX=$((IDX / (NUM_MODELS * NUM_LRS)))
    REMAINDER=$((IDX % (NUM_MODELS * NUM_LRS)))
    MODEL_INDEX=$((REMAINDER / NUM_LRS))
    LR_INDEX=$((REMAINDER % NUM_LRS))

    REL="${REL_VALUES[$REL_INDEX]}"
    MODEL_TAG="${MODEL_TAGS[$MODEL_INDEX]}"
    CONV_SPEC="${CONV_SPECS[$MODEL_INDEX]}"
    MLP_DIM="${MLP_DIMS[$MODEL_INDEX]}"
    REWARD_LR="${REWARD_LRS[$LR_INDEX]}"
    CACHE="${OUT_DIR}/rel${REL}_prof${PROF}_${POLICY}_rollout.pkl"
    ...
    echo "Launching IDX=${IDX}: rel=${REL} model=${MODEL_TAG} reward_lr=${REWARD_LR}"
