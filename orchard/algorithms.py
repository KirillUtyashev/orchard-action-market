...
def spawn_apple(env, p_cell, total_dirt) :
    ...
    total_spawned = int(np.floor(spawn_positions.sum() * dirt_slowed_rate))
    ...
    while Reduced_spawned > 0:
        ...
        positions = np.delete(positions, np.random.choice(len(positions)), axis=0)
        Reduced_spawned -= 1
...
def spawn_apple_selfless_orchard(env, p_cell, total_dirt):
    H, L = env.apples.shape  # rows = width, cols = length
    ...
    total_spawned = int(len(positions))
    dirt_slowed_rate = 1 - ((total_dirt / (H * L)) ** 2)
    total_spawned = int(np.floor(spawn_mask.sum() * dirt_slowed_rate))
    Reduced_spawned = spawn_mask.sum() - total_spawned
    while Reduced_spawned > 0:
        # for i in positions:
        #     if np.random.rand() < 0.5 and Reduced_spawned > 0:
        #         positions = np.delete(positions, i, axis=0)
