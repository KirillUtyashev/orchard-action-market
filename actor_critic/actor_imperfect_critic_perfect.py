...
from helpers.controllers import (
    ...
    AgentControllerActorCriticIndividual,
    ViewController,
)
...
class ActorImperfectCriticPerfect(ActorCritic):
    def __init__(self, config: ExperimentConfig):
        ...
        if self.train_config.budget == 0:
            self.agent_controller = AgentControllerActorCriticIndividual(
                self._agents_list,
                self.critic_view_controller,
                self.actor_view_controller,
            )
        else:
    ...
    def init_agents_for_eval(self):
        ...
        if self.train_config.budget == 0:
            agent_controller = AgentControllerActorCriticIndividual(
                a_list, self.critic_view_controller, self.actor_view_controller
            )
        else:
            agent_controller = AgentControllerActorCritic(
                a_list, self.critic_view_controller, self.actor_view_controller
            )
